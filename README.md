...
- 修复插件被自动停用后反复崩溃重启的问题：停用插件时同步移除依赖引用，不再出现"移走了还按引用找包"的死循环。
- 修复插件市场安装聚合包（如 dsh-web-ui-all）后与内置插件冲突崩溃的问题：检测到重复挂载时自动调整加载顺序，不再误停用插件。
- 服务连续崩溃后停止自动重试并给出引导，不再无限重启空转。

**其他**

- 增强崩溃诊断：覆盖重复路由、服务依赖缺失、浏览器端插件重复加载等更多崩溃形态，自动处理更准确。
