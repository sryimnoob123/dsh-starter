window.__ModuleLoader__.load({ id: '@dsh-desktop/plugin-global-prompt', factory: (require) => {
  ...
  function apply(ctx) {
    ...
    function Page() {
      ...
      React.useEffect(() => {
        fetch('/api/desktop-global-prompt').then((r) => r.json()).then(setState);
        api.workspace.list({}).then((r) => {
          const rows = r.result?.value?.items || [];
      ...
      return React.createElement('div',{className:'ddp'},React.createElement('div',{className:'ddp-tabs'},...Object.keys(labels).map((id)=>React.createElement('button',{key:id,className:'ddp-tab','aria-selected':tab===id,onClick:()=>setTab(id)},labels[id]))),content,tab==='project'?null:React.createElement('div',{className:'ddp-actions'},React.createElement('button',{className:'ddp-btn primary',onClick:save},t('save')),React.createElement('span',{className:'ddp-muted'},status)));
    }
